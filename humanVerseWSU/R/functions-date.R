...
#'
#' Ymd = convertDateStringToFormat(date.strings,"%Y-%m-%d","%m/%d/%Y %H:%M",num=FALSE);
#'
convertDateStringToFormat = function (strvec,to="%Y",from="%Y-%m-%d %H:%M:%S",num=TRUE)
	{
	p.obj = strptime(strvec, format=from );
	o.obj = strftime(p.obj,  format=to );

	if(num) { as.numeric(o.obj); } else { o.obj; }
	}
